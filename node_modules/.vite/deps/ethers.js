...
}, MAPPED, IGNORED, CM, NSM, ESCAPE, GROUPS, WHOLE_VALID, WHOLE_MAP, VALID, EMOJI_LIST, EMOJI_ROOT;
function init() {
	if (MAPPED) return;
...
var AbstractProvider = class {
	...
	*/
	...
	async lookupAddress(address) {
		...
		try {
			const resolver = await new Contract(await EnsResolver.getEnsAddress(this), ["function resolver(bytes32) view returns (address)"], this).resolver(node);
...
var HDNodeWallet = class HDNodeWallet extends BaseWallet {
	...
	*/
	deriveChild(_index) {
		...
		return new HDNodeWallet(_guard, new SigningKey(toBeHex((toBigInt(IL) + BigInt(this.privateKey)) % N$1, 32)), this.fingerprint, hexlify(IR), path, index, this.depth + 1, this.mnemonic, this.provider);
	}
	/**
